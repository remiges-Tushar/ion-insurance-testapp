...
func NewBecknService(db *pgxpool.Pool) *BecknService {
	...
	return &BecknService{db: db, onixBPPCaller: url, bppID: bppID}
}
...
func (s *BecknService) buildOnSearchResponse(reqCtx map[string]any, resources []ResourceRow) map[string]any {
	...
	for _, r := range resources {
		...
		attrs["@context"] = "https://schema.beckn.one/ion/finance/v1"
		attrs["@type"] = "ion:InsuranceProduct"
				...
				"commitments": []any{
					map[string]any{
						...
						"commitmentAttributes": map[string]any{
							...
							"virtualAccount": fmt.Sprintf("8800%010d", policyID),
...
// HandleConfirm validates payment and issues the policy.
func (s *BecknService) HandleConfirm(ctx context.Context, req map[string]any) error {
	...
	// Extract payment proof from commitments[0].commitmentAttributes.payment
	...
	// Generate policy number and certificate URL
	var policyID int64
	var seqNum int
	err := s.db.QueryRow(ctx,
		`SELECT id FROM policies WHERE transaction_id=$1`, txnID,
	).Scan(&policyID)
	if err != nil {
